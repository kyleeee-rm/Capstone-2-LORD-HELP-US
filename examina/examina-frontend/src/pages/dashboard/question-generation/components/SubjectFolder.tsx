...
export default function SubjectFolder() {
  ...
  const { folders, refetch: refetchFolders } = useSubjectFolders(subjectId!);
  ...
  const { selectMode, setSelectMode, selectedIds, toggleSelect, clearSelection } = useSelectMode();
      ...
      <div>
        ...
        <div className="mb-4 flex gap-2 border-b border-border">
          {(["folders", "archived", "trash"] as FolderTab[]).map((tab) => (
            <button
              key={tab}
              onClick={() => { setFolderTab(tab); clearSelection(); }}
