...
export default function DashboardHeader() {
	...
	const featureTitle = routeTitles[currentRoute];
		...
		<>
			<header className="fixed top-0 left-0 right-0 z-40 bg-surface border-b border-border">
				<div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
					{isRoot ? (
						<Button variant="ghost" size="icon" aria-label="Search" render={<Link to="/dashboard/search" />}>
					...
					<div className="flex-1 flex justify-center">
						{isRoot ? (
							...
							<h1 className="text-lg font-bold text-text">{featureTitle}</h1>
						) : (
							<Link to="/dashboard">
								<img
									src={examinaWordmark}
