...
class LearningMaterial(Base):
    ...
    folder_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("subject_folders.folder_id"),
        nullable=False,
        index=True,
    )
    ...
    file_size: Mapped[Decimal] = mapped_column(
        Numeric(precision=12, scale=0),
        nullable=False,
    )
    ...
    teaching_hours: Mapped[Decimal] = mapped_column(
    ...
    lesson_label: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
    )
